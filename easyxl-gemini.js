...
(function() {
    ...
    notesInput.value = localStorage.getItem('easyxl_gemini_notes') || '';
    notesInput.addEventListener('input', () => {
        localStorage.setItem('easyxl_gemini_notes', notesInput.value);
    });
                ...
                onload: function(response) {
                    if (response.status >= 200 && response.status < 300) {
                        try {
                            ...
                            text = (typeof text === 'string' && text.trim()) ? text.trim() : response.responseText;
