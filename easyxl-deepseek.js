// ==UserScript==
// @name         EasyXL (DeepSeek)
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  EasyXL - IXL Solver powered by DeepSeek (OpenAI-compatible API)
// @author       You
// @match        *://*.ixl.com/*
...
(function() {
    ...
    const UI_ID = 'easyxl-deepseek-ui';
    const STORAGE_KEY_API = 'easyxl_deepseek_api_key';
    const STORAGE_KEY_MODEL = 'easyxl_deepseek_model';
    ...
    modelInput.placeholder = 'Model (e.g., deepseek-chat)';
    modelInput.value = localStorage.getItem(STORAGE_KEY_MODEL) || 'deepseek-chat';
    ...
    notesInput.value = localStorage.getItem('easyxl_deepseek_notes') || '';
    notesInput.addEventListener('input', () => {
        localStorage.setItem('easyxl_deepseek_notes', notesInput.value);
    });
    ...
    function sendChatCompletions(url, apiKey, model, systemPrompt, userPrompt, onDone) {
        GM_xmlhttpRequest({
            method: "POST",
            url,
            headers: {
    ...
    parseBtn.onclick = () => {
        ...
        const primaryUrl = 'https://api.deepseek.com/chat/completions';
        const fallbackUrl = 'https://api.deepseek.com/v1/chat/completions';

        sendChatCompletions(primaryUrl, apiKey, model, systemPrompt, userPrompt, (err, response) => {
            if (err) {
            ...
            if (response.status === 404) {
                sendChatCompletions(fallbackUrl, apiKey, model, systemPrompt, userPrompt, (fallbackErr, fallbackResponse) => {
                    if (fallbackErr) {
                        setResult('Request failed (Network Error). Check your API Key or connection.', true);
                        setButtonIdle();
                        return;
    ...
    console.log('EasyXL (DeepSeek) userscript loaded successfully.');
})();
